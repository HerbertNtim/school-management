...
const renderRow = (exam: Exam) => (
  <tr key={exam.subject} className='border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-customPurpleLight'>
    <td className='flex items-center gap-4 p-4'>
      <div className='flex flex-col items-center gap-2'>
        <h3 className='font-semibold'>{exam.subject}</h3>
      </div>
    </td>
    <td className='hidden md:table-cell'>{exam.class}</td>
    <td className='hidden md:table-cell'>{exam.teacher}</td>
    <td className='hidden md:table-cell'>{exam.date}</td>
    <td>
      <div className='flex items-center gap-2'>
        <Link href={`/list/teachers/${exam.id}`}>
...
const ExamsList = () => {
  return (
    <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
      ...
      {/* LIST */}
      <Table columns={examsColumn} renderRow={renderRow} data={examsData} />
      {/* PAGINATION */}
      <Pagination />
    </div>
  )
