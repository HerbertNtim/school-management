...
const renderRow = (schoolClass: Class) => (
  <tr key={schoolClass.name} className='border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-customPurpleLight'>
    <td className='flex items-center gap-4 p-4'>
      <div className='flex flex-col items-center gap-2'>
        <h3 className='font-semibold'>{schoolClass.name}</h3>
      </div>
    </td>
    <td className='hidden md:table-cell'>{schoolClass.capacity}</td>
    <td className='hidden md:table-cell'>{schoolClass.grade}</td>
    <td className='hidden md:table-cell'>{schoolClass.supervisor}</td>

    <td>
      <div className='flex items-center gap-2'>
        <Link href={`/list/teachers/${schoolClass.id}`}>
...
const ClassesList = () => {
  return (
    <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
      ...
      {/* LIST */}
      <Table columns={classesColumn} renderRow={renderRow} data={classesData} />
      {/* PAGINATION */}
      <Pagination />
    </div>
  )
